<?php 
/* SVN FILE: $Id$ */
/* Inbox Test cases generated on: 2010-12-20 14:57:47 : 1292857067*/
App::import('Model', 'Inbox');

class InboxTestCase extends CakeTestCase {
	var $Inbox = null;
	...
	function testInboxInstance() {
		$this->assertTrue(is_a($this->Inbox, 'Inbox'));
	}

	function testInboxFind() {
		$this->Inbox->recursive = -1;
		$results = $this->Inbox->find('first');
		$this->assertTrue(!empty($results));

		$expected = array('Inbox' => array(
			'id'  => 1,
			'sender_id'  => 1,
			'receiver_id'  => 1,
			'receiver_type' => 'user',
	...
	function testToNode() {
		$this->Inbox->recursive = 1;
		$results = $this->Inbox->find('first');
		$node = $this->Inbox->toNode($results);
		$expected = array(
			'id'  => 1,
			'from' => 'Test User',
			'subject' => 'Test',
	...
	function testToNodeMissingModel() {
		try
		{
			$node = $this->Inbox->toNode(array('id' => 1));
			$this->fail('InvalidArgumentException was expected');
		}
		catch (InvalidArgumentException $e)
		{
	...
	function testToNodeMissingKey() {
		try
		{
			$node = $this->Inbox->toNode(array('Inbox' => array('test' => 1)));
			$this->fail('InvalidArgumentException was expected');
		}
		catch (InvalidArgumentException $e)
		{
